...
if($_SERVER['REQUEST_METHOD'] == 'POST'){ 
    if(isset($_POST['create_account'])){
        ...
        $username = $format->validateInput($_POST['username']);
        $email = $_POST['email'];
        ...
        if(empty($username) || strlen($username) > 50){
            $formErrors['usernameError'] = 'Username can not be empty or more than 50 char';
        }
        ...
        $count = $auth->getRowCount(filter: "email = '$email' AND role = '$role'");
        if( $count == 1 ){
        ...
        if($file->check_there_is_file($img_error)){
            ...
            if($file->righte_file_logic($img_error, $file_extension, $allowed_extension, $img_tmp_name, $allowed_mime_type)){
                ...
                $file->uploade_file("../admin/assets/images/users/", $img_tmp_name, $new_img_name);
            }
        ...
        if(empty($formErrors)){
            if( $auth->insert([
                ...
                $$session->set('database_error', 'Wrong try to create account try agian later');
                $path->redirect("../register.php");
            }
        }
